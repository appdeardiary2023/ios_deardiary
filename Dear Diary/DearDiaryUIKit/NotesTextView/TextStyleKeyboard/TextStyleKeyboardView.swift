...
class TextStyleKeyboardView: UIView{
    ...
    let boldButton = TwoStateButton(type: .system)
    let italicButton = TwoStateButton(type: .system)
    let underlineButton = TwoStateButton(type: .system)
    let strikethroughButton = TwoStateButton(type: .system)
    
    //MARK:- Text Alignment Buttons
    let leftAlignButton = TwoStateButton(type: .system)
    let centerAlignButton = TwoStateButton(type: .system)
    let rightAlignButton = TwoStateButton(type: .system)
    
    // MARK:- Text Indent Buttons
    let leftIndentButton = OneStateButton(type: .system)
    let rightIndentButton = OneStateButton(type: .system)
    
    // MARK:- Text Style View
    let titleButton = TextStyleView()
    let headingButton = TextStyleView()
    let bodyButton = TextStyleView()
    let monospacedButton = TextStyleView()
    ...
    override init(frame: CGRect) {
        ...
        keyboardContainerView.backgroundColor = Color.secondaryBackground.shade
        ...
        scrollContainerView.anchor(top: keyboardContainerView.topAnchor, leading: completeStack.leadingAnchor, bottom: nil, trailing: completeStack.trailingAnchor, padding: .init(top: 70, left: -5, bottom: 0, right: 30))
    ...
    func typingAttributeUpdates(currentTextStyle: NotesTextView.TextStyle, isBold: Bool, isItalics: Bool, isUnderline: Bool, hasStrikethrough: Bool, disableLeftIndent: Bool, disableRightIndent: Bool, textColor: UIColor?, highlighColor: UIColor?, textAlignment: NSTextAlignment) {
        ...
        monospacedButton.isActive = currentTextStyle == .monospaced
    ...
    private func setupStyleKeyboard(){
        ...
        // TODO: - Call this method for color stack if needed
//        setupColorStack()
        setupCompleteStack()
    }
    ...
    private func setupTextStyleScrollView(){
        titleButton.label.text = Strings.Note.Text.title
        ...
        headingButton.label.text = Strings.Note.Text.heading
        ...
        bodyButton.label.text = Strings.Note.Text.body
        ...
        monospacedButton.label.text = Strings.Note.Text.monospaced
        monospacedButton.label.font = NotesFontProvider.shared.monospacedFont
        monospacedButton.layer.cornerRadius = buttonCornerRadius
        monospacedButton.constrainHeight(constant: buttonHeight)
        
        titleButton.isActive = false
        headingButton.isActive = false
        bodyButton.isActive = true
        monospacedButton.isActive = false
        ...
        textStyleStack.addArrangedSubview(monospacedButton)
    ...
    private func setupCompleteStack(){
        ...
        let alignmentAndIndentStack = UIStackView(arrangedSubviews: [indentStack, textAlignmentStack, spacer2])
        ...
        // TODO: - Add color stack if needed
//        completeStack.addArrangedSubview(colorStack)
    ...
    private func setupFormatLabel(){
        ...
        formatLabel.textColor = Color.label.shade
        formatLabel.font = Font.title2(.semibold)
        formatLabel.text = Strings.Note.Text.format
        
        addSubview(formatLabel)
        formatLabel.translatesAutoresizingMaskIntoConstraints = false
        formatLabel.anchor(top: topAnchor, leading: safeAreaLayoutGuide.leadingAnchor, bottom: nil, trailing: nil, padding: .init(top: 20, left: 15, bottom: 0, right: 0))
    }
    
    private func setupReturnButton(){
        let crossImage = Image.close.asset?.resize(to: CGSize(width: 24, height: 24))?
            .withTintColor(Color.white.shade)
        returnButton.setImage(crossImage, for: .normal)
        returnButton.backgroundColor = Color.primary.shade
        addSubview(returnButton)
        returnButton.translatesAutoresizingMaskIntoConstraints = false
        returnButton.anchor(top: topAnchor, leading: nil, bottom: nil, trailing: safeAreaLayoutGuide.trailingAnchor, padding: .init(top: 20, left: 0, bottom: 0, right: 15))
        returnButton.constrainHeight(constant: 32)
        returnButton.constrainWidth(constant: 32)
    ...
    private func adjustScrollForTextStyle(style: NotesTextView.TextStyle){
        ...
        case .monospaced:
            activeButtonStart = monospacedButton.frame.origin.x
            activeButtonEnd = activeButtonStart + monospacedButton.frame.width
        }
